...
singleStationStep4UI <- function(id) {
  ...
  return(tagList(
    ...
    conditionalPanel(# show if no station data
      ...
      conditionalPanel(
        condition = "output.qcStatus == 'Done' && output.qualityControlError == '' && output.indexCalculationStatus != 'Done'",
        ns = ns,
        HTML("<div class= 'alert alert-info' role='alert'>
                <span class='glyphicon glyphicon-exclamation-sign' aria-hidden='true'></span>
    ...
    conditionalPanel(
        condition = "output.sectorCorrelationStatus == 'Done' && output.sectorCorrelationError != ''",
        ns = ns,
