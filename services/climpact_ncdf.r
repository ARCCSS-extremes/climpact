...
# Number of data values to process at once. 
# If you receive "Error: rows.per.slice >= 1 is not TRUE", try increasing this to 20. 
# You might have a large grid.
maxvals <- 10
...
create.indices.from.files(infiles,
  ...
  file.template,
  author.data,
  variable.name.map = vars,
  base.range = base.range,
  parallel = cores,
  axis.to.split.on = axis.name,
  climdex.vars.subset = indices,
  thresholds.files = thresholds.files,
  fclimdex.compatible = fclimdex.compatible,
  root.dir = root.dir,
  cluster.type = "SOCK",
  ehfdef = EHF_DEF,
  max.vals.millions = maxvals,
