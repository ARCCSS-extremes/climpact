...
index.calc <- function(progress, metadata, cio, outputFolders, climdexInputParams) {
  ...
  for (i in 1:length(index.list$Short.name)) {
    ...
    if (index.list$Short.name[i] == "hw") { calculate.hw(metadata, cio, outputFolders, pdf.dev, index.list$Short.name[i], index.list$Units[i]); next }
    else if (index.list$Short.name[i] == "spei") { calculate.spei(metadata, cio, outputFolders, pdf.dev, climdexInputParams$custom_SPEI, index.list, trend_file); next }
    else if (index.list$Short.name[i] == "spi") { calculate.spi(metadata, cio, outputFolders, pdf.dev, climdexInputParams$custom_SPEI, index.list, trend_file); next }
    ...
    index.stored <- eval(parse(text = paste0("climdex.", as.character(index.list$Short.name[i]), "(", index.parameter, ")"))) #index.function(cio)
    ...
    plot.call(index.stored, index.name = tmp.index.name, index.units = as.character(index.list$Units[i]), x.label = "Years", sub = tmp.index.def, freq = frequency, metadata, outputFolders, pdf.dev)
  ...
  if (length(climdexInputParams$op.choice) == 0 || length(climdexInputParams$var.choice) == 0) { print("no custom index to calculate", quote = FALSE) } else { calculate.custom.index(cio, metadata, climdexInputParams, outputFolders, pdf.dev, trend_file) }
  dev.off(pdf.dev)
