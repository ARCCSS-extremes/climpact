...
create.dir <- function(user.file, outdir) {
  ...
  if (!file.exists(paste(outinddir, sep = "/"))) { dir.create(paste(outinddir, sep = "/"), showWarnings = FALSE, recursive = TRUE) }
  #; dir.create(paste(getwd(),outinddir,sep="/")) }
  if (!file.exists(paste(outlogdir, sep = "/"))) { dir.create(paste(outlogdir, sep = "/"), showWarnings = FALSE, recursive = TRUE) }
  #; dir.create(paste(getwd(),outlogdir,sep="/")) }
  if (!file.exists(paste(outjpgdir, sep = "/"))) { dir.create(paste(outjpgdir, sep = "/"), showWarnings = FALSE, recursive = TRUE) }
  #; dir.create(paste(getwd(),outjpgdir,sep="/")) }
  if (!file.exists(paste(outtrddir, sep = "/"))) { dir.create(paste(outtrddir, sep = "/"), showWarnings = FALSE, recursive = TRUE) }
  #; dir.create(paste(getwd(),outtrddir,sep="/")) }
  if (!file.exists(paste(outqcdir, sep = "/"))) { dir.create(paste(outqcdir, sep = "/"), showWarnings = FALSE, recursive = TRUE) }
  #; dir.create(paste(getwd(),outqcdir,sep="/")) }
  if (!file.exists(paste(outthresdir, sep = "/"))) { dir.create(paste(outthresdir, sep = "/"), showWarnings = FALSE, recursive = TRUE) }
  #; dir.create(paste(getwd(),outthresdir,sep="/")) }
  if (!file.exists(paste(corrdir, sep = "/"))) { dir.create(paste(corrdir, sep = "/"), showWarnings = FALSE, recursive = TRUE) }
  #; dir.create(paste(getwd(),corrdir,sep="/")) }
